...
const Products = () => {
    ...
    const getAllProducts = async () => {
        try {
            const { data } = await axios.get(`https://full-stack-ecommernce-app-backend.onrender.com/api/v1/product/product-list/${page}`);
            setLoading(false)
                        ...
                        {products?.map((p) => (
                            <Link key={p._id} to={`/dashboard/admin/product/${p.slug}`} className="mt-3 mb-3 product-link">
                                <div className="card m-3" style={{ width: '18rem', borderRadius: '4px' }} key={p._id}>
                                    <img src={`https://ecom-final-fixed-backup.onrender.com/api/v1/product/product-photo/${p._id}`} className='card-img-top ' alt={p.name} style={{ width: "100%", height: '300px', objectFit: 'cover', padding: "1px" }} />
