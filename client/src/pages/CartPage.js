...
const CartPage = () => {
    ...
    /* ── deduplicate + ensure every cart item has cartQty starting at 1 ── */
    useEffect(() => {
        if (!cart?.length) return;

        // Deduplicate by _id, keeping the last occurrence
        const seen = new Map();
        cart.forEach(item => seen.set(item._id, item));
        const deduped = Array.from(seen.values()).map(item => ({
            ...item,
            cartQty: item.cartQty ?? 1,
        }));

        const changed =
            deduped.length !== cart.length ||
            deduped.some((f, i) => f.cartQty !== cart[i]?.cartQty);
